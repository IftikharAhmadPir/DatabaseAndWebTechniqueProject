...
namespace DBWTechniqueBackend
{
    public class BusinessLogic
    {
        ...
        {
            var List = DataLayerObj.GetProxyFromDatabase();
            foreach (var value in List)
            {
                DataLayerObj.updateProxyStatus(value.ID, PingFunction(value.Ip,value.Port), DateTime.Now);
            }
        }
            ...
            try
            {
                for(int i=1;i<=iterations;i++)
                {
                    ...
                    if(json.SelectToken("type").ToString().Equals("http"))
                        DataLayerObj.storeProxyFromProviderList(json.SelectToken("ipPort").ToString(), json.SelectToken("ip").ToString(), json.SelectToken("port").ToString(), json.SelectToken("country").ToString(), json.SelectToken("type").ToString(),PingFunction(json.SelectToken("ip").ToString(), json.SelectToken("port").ToString()), "GimmeProxy");
                }
            }
            catch(Exception)
            {
            ...
            try
            {
                for (int i = 1; i <= iterations; i++)
                {
                    ...
                    if (json.SelectToken("protocol").ToString().Equals("http"))
                        DataLayerObj.storeProxyFromProviderList(json.SelectToken("ip").ToString() + json.SelectToken("port").ToString(), json.SelectToken("ip").ToString(), json.SelectToken("port").ToString(), json.SelectToken("country").ToString(), json.SelectToken("protocol").ToString(), PingFunction(json.SelectToken("ip").ToString() , json.SelectToken("port").ToString()), "GetProxyList");
                }
            }
            catch (Exception)
            {
            ...
            try
            {
                for (int i = 1; i <= iterations; i++)
                {
                    ...
                    if (json.SelectToken("data[0].type").ToString().Equals("http"))
                        DataLayerObj.storeProxyFromProviderList(json.SelectToken("data[0].ipPort").ToString(), json.SelectToken("data[0].ip").ToString(), json.SelectToken("data[0].port").ToString(), json.SelectToken("data[0].country").ToString(), json.SelectToken("data[0].type").ToString(), PingFunction(json.SelectToken("data[0].ip").ToString(), json.SelectToken("data[0].port").ToString()), "PubProxy");
                }
            }
            catch (Exception)
            {
        ...
        public int PingFunction(string Ip,string Port)
        {
            int is_success = 0;
            try
            {
                HttpWebRequest Request = (HttpWebRequest)WebRequest.Create("https://www.google.com/");
                IWebProxy ProxyAddress = new WebProxy(Ip, Convert.ToInt32(Port));
